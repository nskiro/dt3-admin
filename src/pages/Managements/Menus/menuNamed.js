...
class MenuEditForm extends Component {
    ...
    onLoadAccessLinks = (v) => {
        axios.get(ac_get_link, { params: v })
            .then((res) => {
                let rs = res.data;
                if (rs.valid) { this.setState({ access_link_data: rs.data }); } else {
    ...
    loadMenus = (v) => {
        axios.get(menu_get_link, { params: v })
            .then((res) => {
                let rs = res.data;
                if (rs.valid) {
                    let menu_data = [];
    ...
    onLoadData = (treeNode) => {
        ...
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                ...
                axios.get(menu_get_link, { params: { menu_parent: treeNode.props.eventKey } })
                    .then((res) => {
                        /*
                        let rs = res.data;
                        if (rs.valid) {
                        ...
                        <Row className="show-grid">
                            < Col md={6} sm={6} xs={6} >
                                <FormItem label={'MENU TREE'}>
                                    {getFieldDecorator('menu_parent', { initialValue: this.props.data.menu_parent }, {})
                                ...
                                <Row className="show-grid">
                                    < Col md={12} sm={12} xs={12} >
                                        <FormItem label={'MENU PARENT'}>
                                            {getFieldDecorator('menu_parent_label', { initialValue: this.state.menu_parent_label }, {})
                                ...
                                <Row className="show-grid">
                                    < Col md={12} sm={12} xs={12} >
                                        <FormItem label={'MENU LABEL'}>
                                            {getFieldDecorator('menu_label', { initialValue: this.props.data.menu_label }, {})
...
class MenuNamed extends Component {
    ...
    onShowEditForm = (e) => { this.setState({ modalvisible: true }); }
    saveFormRef = (formRef) => { this.formRef = formRef; }
    onHandleCancel = (e) => { this.setState({ modalvisible: false }) }
    ...
    render() {
        ...
        return (
            <div>
                <div>
                    <Button value='new' size={button_size} onClick={this.onShowEditForm} >NEW</Button>
                    <Button value='edit' size={button_size} onClick={this.onShowEditForm} >EDIT</Button>
                    <Button size={button_size} onClick={this.onEnableMenu}>ENABLE</Button>
                    <Button size={button_size} onClick={this.onDisableMenu}>DISABLE</Button>
                </div>
                ...
                <Table
                    ...
                    onRow={(record) => {
                        return {
                            onClick: () => { this.setState({ menu_selected: record }); },       // click row
