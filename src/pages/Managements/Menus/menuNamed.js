                ...
                <Row className="show-grid">
                  <Col md={12} sm={12} xs={12}>
                    <FormItem label={'MENU PARENT'}>
                      {getFieldDecorator('menu_parent_label', { initialValue: this.state.data_ref.menu_parent_label }, )
                ...
                <Row className="show-grid">
                  <Col md={12} sm={12} xs={12}>
                    <FormItem label={'ACCESS LINK'}>
                      {getFieldDecorator('access_link', { initialValue: data_ref.access_link_id })(<Select
                        showSearch
                        name="access_link"
...
class MenuNamed extends Component {
  ...
  onHandleLoadMenus = () => {
    axios
      ...
      .then(res => {
        ...
        } else { this.setState({ rows: [] }); alert(rs.message); }
      })
      .catch(err => { console.log(err); })
  }
  ...
  onEnableMenu = (e) => {
    ...
    let id = data._id;
    axios.post(menu_enable_link + `${id}`, data)
      .then((res) => {
        let rs = res.data;
        if (rs.valid) {
          this.onHandleLoadMenus();
  ...
  onDisableMenu = (e) => {
    ...
    axios.post(menu_disable_link + `${id}`, data)
      .then((res) => {
        let rs = res.data;
        if (rs.valid) {
          this.onHandleLoadMenus();
    ...
    return (
      <div>
        <div>
          ...
          <Button icon="edit" value="edit" size={button_size} disabled={_.isEmpty(this.state.menu_selected) ? true : false} onClick={this.onShowEditForm}> edit </Button>
          <Divider type="vertical" />
          <Popconfirm title="Are you sure enable ?" onConfirm={this.onEnableMenu} okText="Yes" cancelText="No">
            <Button icon="check-circle" size={button_size} disabled={_.isEmpty(this.state.menu_selected) ? true : false}> enable </Button>
          </Popconfirm>
          <Divider type="vertical" />
          <Popconfirm title="Are you sure disable ?" onConfirm={this.onDisableMenu} okText="Yes" cancelText="No">
            <Button icon="close-circle" type="danger" size={button_size} disabled={_.isEmpty(this.state.menu_selected) ? true : false}> disable </Button>
          </Popconfirm>
          <Divider type="vertical" />
          <Button icon="retweet" size={button_size} onClick={this.onHandleLoadMenus}> refesh </Button>
        </div>

        <Table
          rowKey={'_id'} size="small" bordered style={{ marginTop: '5px' }} columns={this.state.columns} dataSource={this.state.rows}
