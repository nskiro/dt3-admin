                            ...
                            < Col md={12} sm={12} xs={12} >
                                <FormItem label={'Access link description'}>
                                    {getFieldDecorator('des', { initialValue: this.props.data.des }, {})
                                        (<TextArea placeholder='access link desciption something !' autosize={{ minRows: 2, maxRows: 6 }} />)}
                                </FormItem>
                            </Col>
                        </Row>
    ...
    constructor(props, context) {
        ...
        this.state = {
            ...
            columns: [
                ...
                {
                    title: 'STATUS', dataIndex: 'record_status', key: 'record_status',
                    render: (text, row, index) => {
                       
                        if (text === 'O') { return 'Đang hoạt động'; }
    ...
    onHandleCreateMenu = (e) => {
        ...
        form.validateFields((err, values) => {
            ...
            if (values.id) {
                ...
                axios.post(ac_update_link + `${values.id}`, data)
                    .then((res) => {
                        console.log(res.data);
                        let rs = res.data;
                        if (rs.valid) {
        ...
        return (
            <div>
                ...
                <Table
                    ...
                    onRow={(record) => {
                        return {
                            onClick: () => { this.setState({ access_link_selected: record }); },       // click row
